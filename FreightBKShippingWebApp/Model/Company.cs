...
namespace FreightBKShippingWebApp.Model
{
    ...
    public class Company
    {
        ...
        public string CompanyGstNo = string.Empty;



        [Column("company_gstin")]
        //[RegularExpression(@"^(\d{2}[A-Z]{5}\d{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1})?$",
            //ErrorMessage = "Enter valid GSTIN (15 chars).")]
        public string? Gstin { get => CompanyGstNo;
            set
            {
                CompanyGstNo = value?.ToUpper() ?? string.Empty;
                Panno = (!string.IsNullOrEmpty(CompanyGstNo) && CompanyGstNo.Length >= 12)
                    ? CompanyGstNo.Substring(2, 10)
                    : string.Empty;
            }
        }
        ...
        public string? Panno { get => CompanyPan;
            set => CompanyPan = value?.ToUpper();
        }
